...
// Already deployed Controller address
const CONTROLLER_ADDRESS = "0x51670c4b400051Ace5c01aAa41d4a1d3eA5C445a";

async function main() {
  const [deployer] = await ethers.getSigners();
  const deployerAddress = await deployer.getAddress();

  console.log("Resuming Index Protocol deployment to Polygon mainnet...");
  ...
  console.log("\nUsing existing Controller at:", CONTROLLER_ADDRESS);
  const controller = await deployHelper.core.getController(CONTROLLER_ADDRESS);

  // 2. Deploy SetTokenCreator
  console.log("\n2. Deploying SetTokenCreator...");
  const setTokenCreator = await deployHelper.core.deploySetTokenCreator(CONTROLLER_ADDRESS);
  ...
  const integrationRegistry = await deployHelper.core.deployIntegrationRegistry(CONTROLLER_ADDRESS);
  ...
  const basicIssuanceModule = await deployHelper.modules.deployBasicIssuanceModule(CONTROLLER_ADDRESS);
  ...
  const streamingFeeModule = await deployHelper.modules.deployStreamingFeeModule(CONTROLLER_ADDRESS);
  ...
  const tradeModule = await deployHelper.modules.deployTradeModule(CONTROLLER_ADDRESS);
  ...
  const debtIssuanceModuleV2 = await deployHelper.modules.deployDebtIssuanceModuleV2(CONTROLLER_ADDRESS);
  ...
  const wrapModuleV2 = await deployHelper.modules.deployWrapModuleV2(CONTROLLER_ADDRESS, WMATIC);
