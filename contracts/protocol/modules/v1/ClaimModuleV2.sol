 ...
 *
 * Module that enables managers to claim tokens from external protocols that require additional claim data parameters.
 * Extends ClaimModule functionality by allowing arbitrary bytes data to be passed to claim calls.
 */
contract ClaimModuleV2 is ModuleBase {
    ...
    event RewardClaimed(
        ...
        IClaimV2Adapter indexed _adapter,
        uint256 _amount,
        bytes _claimData
    );
     ...
     * @param _claimData            Bytes data passed to claim function
     */
    function claim(
        ISetToken _setToken,
        address _rewardPool,
    ...
    {
        _claim(_setToken, _rewardPool, _integrationName, _claimData);
    }
        ...
        onlyValidCaller(_setToken)
    {
        ...
        require(poolArrayLength == _claimData.length, "Array length mismatch");
        
        for (uint256 i = 0; i < poolArrayLength; i++) {
            _claim(_setToken, _rewardPools[i], _integrationNames[i], _claimData[i]);
        }
    }
     ...
     *
     * @param _setToken             Address of SetToken
     * @param _anyoneClaim          Boolean indicating if anyone can claim
     */
    ...
    function removeModule() external override {
        ...
        delete anyoneClaim[setToken];

        // Explicitly delete all elements for gas refund
        address[] memory setTokenPoolList = rewardPoolList[setToken];
        for (uint256 i = 0; i < setTokenPoolList.length; i++) {
            address[] storage adapterList = claimSettings[setToken][setTokenPoolList[i]];
            for (uint256 j = 0; j < adapterList.length; j++) {
                address toRemove = adapterList[j];
                claimSettingsStatus[setToken][setTokenPoolList[i]][toRemove] = false;
                delete adapterList[j];
            }
            delete claimSettings[setToken][setTokenPoolList[i]];
        }

        for (uint256 i = 0; i < rewardPoolList[setToken].length; i++) {
            address toRemove = rewardPoolList[setToken][i];
            rewardPoolStatus[setToken][toRemove] = false;
            delete rewardPoolList[setToken][i];
        }
        delete rewardPoolList[setToken];
    }
    ...
    function getRewards(
        ...
        returns (uint256)
    {
        IClaimV2Adapter adapter = _getAndValidateIntegrationAdapter(_setToken, _rewardPool, _integrationName);
     ...
     * @param _claimData            Bytes data passed to claim function
     */
    function _claim(ISetToken _setToken, address _rewardPool, string calldata _integrationName, bytes calldata _claimData) internal {
        ...
        emit RewardClaimed(_setToken, _rewardPool, adapter, finalRewardsBalance.sub(initRewardsBalance), _claimData);
    }
     ...
     * @param _integrationName      ID of claim module integration (mapping on integration registry)
     * @return IClaimV2Adapter      The claim adapter
     */
